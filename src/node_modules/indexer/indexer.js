...
function indexFile(code, file, verbose)
{
    ...
    function add(name, scope) {
        ...
        if (locations[0][2] === DEFINITION) {
            ...
            for (var idx=scope.scopeStack.length - 1; idx>=0; --idx) {
                ...
                if (def) {
                    if (def.location[2] === DEFINITION) {
                        newDef = false;
                        defObj = def;
                        break;
                    } else if (def.location[2] !== REFERENCE && (!defObj || def.location[2] < defObj.location[2])) {
                        newDef = false;
                        defObj = def;
                    }
                }
    ...
    for (var s=0; s<scopes.length; ++s) {
        ...
        if (scope.count) {
            var pendingReferences = [], pendingMaybeReferences = [];
            for (var name in scope.objects) {
                ...
                switch (scope.objects[name][0][2]) {
                case DEFINITION:
                    add(name, scope);
                    break;
                case MAYBE_REFERENCE:
                    pendingMaybeReferences.push(name);
                    break;
                case REFERENCE:
                    pendingReferences.push(name);
                    break;
                }
            }
            var n;
            for (n=0; n<pendingMaybeReferences.length; ++n) {
                add(pendingMaybeReferences[n], scope);
            }
            for (n=0; n<pendingReferences.length; ++n) {
                add(pendingReferences[n], scope);
            }
        }
    }
